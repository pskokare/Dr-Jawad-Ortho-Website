...
async function loadNavbar() {
    try {
        // Check if we're in the Terminals folder and adjust path accordingly
        const navbarPath = window.location.pathname.includes('/Terminals/') || window.location.pathname.includes('/LatestNews/') || window.location.pathname.includes('/RecentPost/') ? '../components/navbar.html' : 'components/navbar.html';
        const response = await fetch(navbarPath);
        const navbarHTML = await response.text();
        document.getElementById('navbar-container').innerHTML = navbarHTML;

        // Fix paths if we're in Terminals, LatestNews, or RecentPost folder
        if (window.location.pathname.includes('/Terminals/') || window.location.pathname.includes('/LatestNews/') || window.location.pathname.includes('/RecentPost/')) {
            // Fix image paths
            ...
            // Fix all navigation links that don't start with Terminals/, LatestNews/, or RecentPost/
            const navLinks = document.querySelectorAll('#navbar-container a[href]');
            navLinks.forEach(link => {
                const href = link.getAttribute('href');
                if (href && !href.startsWith('http') && !href.startsWith('#') && !href.startsWith('../')) {
                    if (window.location.pathname.includes('/Terminals/')) {
                        if (href.startsWith('Terminals/')) {
                            // If already in Terminals folder, remove Terminals/ prefix
                            link.href = href.replace('Terminals/', '');
                        } else {
...
async function loadFooter() {
    try {
        // Check if we're in the Terminals folder and adjust path accordingly
        const footerPath = window.location.pathname.includes('/Terminals/') || window.location.pathname.includes('/LatestNews/') || window.location.pathname.includes('/RecentPost/') ? '../components/footer.html' : 'components/footer.html';
